...
- *International Driving Permit ✅ (sixth domain):* `ID_PAYLOAD` expanded 90 → **202** (full coverage). Each added row carries a verified driving side (L/R, cross-checked against authoritative sources), plus IDP requirement, road-traffic-convention membership (`[]` where a country isn't a party — the tool then shows generic IDP/translation advice), signage style and units. Captures distinctive facts (Eastern-Caribbean temporary-permit rules, Samoa's 2009 switch to the left, Macau driving opposite mainland China, Vanuatu as the right-driving Pacific exception). The `ID_DATA` projection still injects name/flag from the registry.
- *Driving Rules ✅ (seventh domain):* `DR_DATA` expanded 53 → **202** (full coverage), authored region-by-region (South America, Caribbean & Central America, Europe, Asia & Middle East, Oceania, Africa). The two legally-precise fields — per-country **speed limits** (urban/rural/motorway) and **blood-alcohol limits** — were web-sourced (WHO road-safety profiles, national codes, the Wikipedia drink-driving-by-country table, Jun 2026); side/IDP/emergency reuse the registry-backed datasets. Captures zero-tolerance regimes, Gulf/Islamic alcohol bans, RHD-cars-on-the-right hazards (Myanmar, Vanuatu, Liberia) and gravel-road/wildlife warnings. Lower-confidence micro-state figures carry an "approximate — confirm locally" note. Behind the tool's verify-locally disclaimer.
- *Quiet Hours / Etiquette ✅ (eighth domain):* `QH_DATA` expanded 20 → **202** (full coverage). Soft cultural data (quiet hours, Sunday/holiday trading, noise, dress, photography, plus a "good to know" note) authored region-by-region. To make the fill visible, the QH country picker (`qhRenderCountryList`/`qhCountry`) was **repointed from the 53-country tipping list to `QH_DATA` projected through `TT_COUNTRIES`**. Captures high-value norms (Israel's Shabbat, Bali's Nyepi, Sri Lanka/Myanmar Buddha-image rules, Singapore's fineable offences, Tonga's Sunday-closure law, Fiji's village/kava protocol, Rwanda's Umuganda, Ethiopian Orthodox custom, Madagascar's *fady* taboos, and the widespread no-photographing-officials rule).
- *Tipping ✅ (ninth domain):* `TC_PAYLOAD` expanded 53 → **202** (full coverage), authored region-by-region with full eight-venue detail (restaurant/bar/taxi/delivery/housekeeping/porter/tour/spa) plus a service-charge flag and a cultural note. A terse `_tcv()` venue-map builder keeps the added rows readable. Captures safari/gorilla guide tipping, North-African/Levant baksheesh, Gulf service-charge-plus-cash norms, Himalayan trek-guide tipping, the tipping-not-customary Pacific (with resort "staff fund" boxes) and USD-ized economies. **Side effect:** the Driving Rules and Tipping-Etiquette country pickers iterate `TC_COUNTRIES`, so they were silently capped at 53 — expanding tipping lifts both to the full 202, finally surfacing the `DR_DATA` fill in the UI (the etiquette guide degrades gracefully where its `TG_ETIQUETTE` layer has no entry).
- *VAT refund ✅ (tenth domain):* `VR_PAYLOAD` expanded 42 → **202** (full coverage). The **49** countries that genuinely run a tourist VAT/GST refund are full `scheme:true` entries (the 7 remaining EU members plus non-EU schemes — Liechtenstein via Switzerland, Monaco via France, Serbia, Russia, Vietnam); the other **153** are explicit "no tourist VAT refund" entries carrying the local sales-tax rate and a one-line reason, so any lookup gives a definitive answer. A `_vrNo()` helper keeps the no-scheme rows DRY and the picker now sorts each region alphabetically. Introducing/non-operational schemes (Saudi Arabia, Philippines, India, Peru) are flagged rather than assumed.
- *ATM fees ✅ (eleventh domain — scoped fill):* `AF_PAYLOAD` expanded 15 → **57** major destinations (+ the Eurozone/Euronet pseudo-entry), each with cash currency and a typical *local* operator surcharge; `AF_CUR` gained 23 currencies so the new presets resolve a valid dropdown option. Per the chosen scope this is a **major-destinations** fill, not full 202 — volatile-currency economies are deliberately omitted rather than given a false-precise figure the fee field can't caveat.
- *Coverage status:* **every identity-carrying destination domain is now at full 202-country coverage** except ATM fees, which is a deliberate major-destinations (57) fill given the volatility of operator-fee amounts. Power Plug, Emergency, Visa, Vaccination, Customs, International Driving Permit, Driving Rules, Quiet Hours, Tipping and VAT Refund all span the full registry.
...
**Phase 6 — Finish existing depth** ✅ *Done*
- After Phases 0–5 the registry covered all 202 countries (every sovereign state plus PR/GU/PF/HK/MO/TW and Kosovo), and every per-country domain was at full coverage **except** ATM fees (57) and the destination list (86 of 202 countries had a city). Phase 6 closes both, at the same best-effort/dated/disclaimed bar.
- *Destinations → full 202 (`6a`):* added **one anchor city** (capital + IANA timezone + registry flag) for each of the **116** registry countries that had no destination, lifting `TT_DESTINATIONS` from 216 → **332** so every country resolves. Rows are minimal by design — `inspire:false` and **no `cost`** — so the **Destination Picker stays at 113 curated rows** and **Cost of Living stays at 41**; only the **Time Zone / Meeting Planner / Jet Lag** list (`TZ_CITIES`) grows 130 → 246 (appended via `_tzi` 130–245, existing order preserved) and `resolveDestination()` now resolves the new capitals everywhere (e.g. "San Juan" → PR, "Pristina" → XK, "Port Vila" → VU). All 116 IANA zones validated; no `id`/`_tzi` collisions.
- *ATM fees → full 202 (`6b`):* expanded `AF_PAYLOAD` 57 → **202** countries (+ the Eurozone/Euronet pseudo-entry). Each row carries the cash currency (from `TT_COUNTRIES.currency`) and a typical local operator surcharge; `AF_CUR` gained **95** currencies so every preset resolves a valid dropdown option, and `AF_NO_DEC` gained the new whole-number currencies. For the **37** cash-based / volatile / limited-ATM economies (e.g. Venezuela, Zimbabwe, Lebanon, Syria, Iran, Russia, Afghanistan, North Korea, several Pacific micro-states) the surcharge is left at 0 with a per-country **`note` caveat** ("ATMs unreliable — bring cash") that now renders above the ATM tool's smart hint, rather than a false-precise figure. Verified: all 202 link to the registry, no duplicate codes, every currency present in `AF_CUR`.
- *Coverage status:* **all 202 registry countries now have at least one destination and full ATM-fee coverage** — every per-country domain and the destination list span the full registry.

**Phase 7 — New data domains** ✅ *(complete)*
- Per the product decision (Jun 2026), the next net-new authoring effort adds four domains across the 202 countries, in priority order: **Languages & phrases → Public holidays → eSIM & mobile data → Safety & advisories** (tap-water potability and LGBTQ+/solo-traveller safety fold into the last). Same best-effort/dated/disclaimed bar; safety/health rows carry the strong disclaimers those domains already use. One domain per PR (established cadence).
- *Safety & advisories ✅ (fourth domain):* new **Safety & Advisories** applet for all **202** countries (`SAFE_DATA`), folding in the requested extras. Each country has an overall advisory **level 1–4** (normal precautions → avoid all travel; distribution 90/60/31/21), common risks & scams, **tap-water potability** (safe/caution/unsafe — 54/64/84), a **solo & women-traveller** note, and **LGBTQ+ legal/social status** (open/legal/discreet/restrict/illegal — 48/64/15/15/60) with severe-penalty notes where applicable. Colour-coded hero by advisory level, registry-backed picker, trip auto-detect via `resolveDestination()`, print, and a strong "orientation only — not an official advisory or legal/medical advice; check your government's advisory and a travel clinic" disclaimer. All 202 mapped; full inline JS passes `node --check`.
- *eSIM & mobile data ✅ (third domain):* new **eSIM & Mobile Data** applet for all **202** countries (`ESIM_DATA`), covering all four requested facets: eSIM availability (152 full / 40 partial / 10 none) with a typical tourist per-GB price band and the global eSIM apps (Airalo/Holafly/aloSIM/Saily); typical urban network generation + coverage caveats; local prepaid-SIM rules (no-registration / passport-registration / impractical) with main operators; and a censorship/VPN "heads up" for the **33** countries with notable restrictions (China's Great Firewall, Gulf VoIP blocks, Turkey's device-registration trap, Iran/Turkmenistan/North Korea, etc.). Registry-backed picker, trip auto-detect via `resolveDestination()`, print support, verify-before-travel disclaimer. All 202 mapped; full inline JS passes `node --check`.
- *Public holidays ✅ (second domain):* new **Public Holidays** applet covering all **202** countries, with a movable-feast **date engine** that computes dates for any year — Western Easter (Computus) and offsets (Good Friday/Ascension/Whit/Corpus Christi), Orthodox Easter (Julian→Gregorian), the Islamic Eids/Mawlid/Ashura via a tabular Hijri converter, Chinese/Lunar New Year (astronomical table 2024–2035), nth-weekday rules (US Thanksgiving, UK bank holidays, etc.), and a per-year table for Israel's Hebrew-calendar holidays. Holidays governed by lunar/Hebrew/Hindu calendars carry an `≈` "approximate" marker + caveat. The applet auto-detects the country from the active trip via `resolveDestination()`, has a year stepper, "next up" highlighting and print support. Engine validated against known dates; all 202 countries produce holidays with no errors across 2024–2030; full inline JS passes `node --check`.
- *Languages & phrases ✅ (first domain):* added a country→language layer to **all 202** `TT_COUNTRIES` entries — `langs` (official/main languages, display names) and an optional `pb` (the Language Phrasebook code to auto-open; set for the **123** countries whose dominant everyday language is one of the phrasebook's 25, including standard mutually-intelligible groupings cs↔sk, hr↔bs/sr/me, id↔ms, hi↔ur). The existing Phrasebook already had a bespoke 60-string `lpLangForDest()` matcher (~25 countries); it now **routes through `resolveDestination()` → `TT_COUNTRIES[cc].pb` first** (alias- and bare-city-aware, so it covers all 202 — e.g. Bora Bora→French via PF, San Juan→Spanish via PR, Bratislava→Czech), with the legacy map kept as a no-regression fallback. Added a self-check guardrail (every country has `langs`; any `pb` must resolve to a real `LP_LANGS` entry). Verified: 202/202 mapped, 15/15 auto-detect cases pass, full inline JS passes `node --check`.
