...
**Phase 3 — Coverage expansion / payload fill** 🔶 *In progress*
...
- *Customs ✅ (fifth domain, EU bloc):* `CDH_COUNTRIES` 42 -> **54** by adding the 12 remaining EU members via the shared `cdhEU()` harmonised-rules template (accurate — all 27 EU states now covered). The ~148 **non-EU** countries are intentionally NOT mass-authored: each needs bespoke duty-free allowances, prohibited/restricted lists and cash thresholds — dense, volatile, per-country legal data that warrants proper sourcing and human review rather than best-effort search synthesis.
- *Coverage status of remaining domains:* VAT refund (~42, only ~50 countries run tourist schemes), ATM fees (16, operator-fee amounts volatile), tipping (53, subjective) are complete-by-design or low-confidence and left as-is.
